...
- **Plain PR titles**: user-facing titles and labels are work descriptions only (never `pass N` / `full-pass-N` / `poteto pass`). Encoded in lua-kit `poteto-lua`, typescript-kit `poteto-typescript`, python-kit `poteto-python`, and go-kit `poteto-go`.
...
See also [lua.md](lua.md), [python.md](python.md), [go.md](go.md), [pr-workflow.md](pr-workflow.md), [evidence-standard.md](evidence-standard.md).
