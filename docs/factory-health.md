...
| TS / UI | Pack **typescript-kit** 0.5.0 (`poteto-typescript` standing scorecard, `ts-rg-gate` + `ts-strict-gate` + `ts-runtime-gate` + `ts-oxlint-gate` + global-fetch/URL/env boundaries + env-schema/typed-parse templates + `ts-kit-selfcheck`) + Control-Glass product gates |
