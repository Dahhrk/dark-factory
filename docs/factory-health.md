...
- **Plain PR titles**: user-facing titles and labels are work descriptions only (never `pass N` / `full-pass-N` / `poteto pass`). Encoded in lua-kit `poteto-lua` and typescript-kit `poteto-typescript`.
...
| TS / UI | Pack **typescript-kit** 0.6.0 (`poteto-typescript` standing scorecard, `ts-rg-gate` + `ts-strict-gate` + `ts-runtime-gate` + `ts-oxlint-gate` pinned offline-capable live oxlint + global-fetch/URL/env boundaries + env-schema/typed-parse templates + `ts-kit-selfcheck`) + Control-Glass product gates |
