...
“I almost never look at code anymore” is not abdication. She moved the check from her eyes into architecture + CI + verification. The 600+ Dune PRs are the construction cost.

## Maven chapter map

| Time | Chapter | What she does |
|------|---------|----------------|
| 00:00 | Intro | Netflix EM → Meta React Compiler → Cursor / Grok Bot. Still on React core. |
...
| 00:16 | pstack | Skills so agents stop hallucinating a process. Named after Gary Tan’s gstack; no relation. |
| 00:20 | Evals | Unit tests for skills. Blinded. Cross-model judge. Loop to a 10. |
| 00:26 | Cloud / Benny | Local eyes → cloud computer. Benny repros Slack bugs; sometimes the fix is already on main. |
| 00:32 | Organic architecture | Vibe-coded greenfield is the danger. Big-company problems are now everyone’s. |
...
2. **Cloud parallel.** Environment is set. The same loop runs without you in the chair. “If you set the environment up, the investment pays huge dividends.”
3. **Auto-merge.** She woke to ~20 PRs on main and reviewed the already-landed branch. Contents were good.

Load-bearing line: if you cannot trust **one** agent’s output, you cannot run a hundred. Same as an EM who does not trust the team — you spend the day looking over shoulders.

There is **no shortcut up this curve**. “This is about the level of trust you have in your own agents.” Stage-1 people who spawn 100 cloud agents waste tokens and pay for it.
...
Week one at Cursor: perf work for a release in seven days. Brand-new codebase. She screenshots Chrome traces. The agent says “probably this” with total confidence. She applies it. Wrong.

That loop — instruct, receive, **you** build, it fails, you paste the error — is why parallelism never starts. The human is the 3-minute egg (see [Loops You Can Trust](loops-you-can-trust.md)).

**control glass** was the first skill she wrote. “Glass” is the internal name for Cursor’s agent window. The skill is markdown that teaches CDP and Apple’s simulator-control tools: launch the app, click, screenshot, read the console. She says the code is not interesting; your agent can write it. The idea is: **give the agent the same eyes you have.**

Verification does not guarantee *good* code. It does let the agent write code that **works**. That is a large step for trust.

Then the agent could drive the product and still had no idea what the product *was*. Slack: “left sidebar jank” plus a screenshot of `???`. Without a map it clicks at random.
...
- Keyboard shortcuts
- Selectors / DOM attributes the agent should grab (`data-component`, ARIA — “like the attributes”)

Four H2s per file: Sub-features → user POV → Driving it with `control-glass` → Gotchas. Shared **Driving conventions** on `features/README.md`. Sharpest rule: `wait-settle` is a short renderer wait, not “the turn finished.”
...
Same talk, handwritten slide — **how do I trust my agents more?** Three levers: verification, high-quality skills (pstack), agent-friendly architecture (greenfield vs brownfield). Evals annotated on the first two.

## Evals (the part the essays barely show)

In her head, an eval is a **unit test for a skill**.

1. A coordinator agent writes a rubric (what “good” means).
2. Many subagents run the skill on organic-looking tasks.
3. Directory names are sanitized so they do **not** know they are being scored. Agents detect evals and change behavior — same as a salesperson who is suddenly polite when the manager sits in.
...
Skill maintenance is hard. It needs taste and a good **backseat driver** — the pair-programming instinct that asks “why did you do it that way?”
...
Live Slack (`#glass-oncall-assistant` → `#issues-glass`): tyson reports panel focus reopening a blank Apps sidebar (video attached). Benny replies 🐸 **Reproduced but already fixed on main** — “reproduced on the prior commit, gone on the fix.” `[view cloud agent]` is in the message. No competing PR. They needed a new build. An hour of sitting with an agent, gone. She frames the win as a **team** level-up, not a personal flex.

Do not skip to a hundred Bennys from stage 1.

## Organic architecture (why greenfield is the trap)

“Big-company problems are now everyone’s problems.”
...
Grok Bot itself: stood up extremely fast. Prototype, vibe-coded, humans not reading. Agents then solve each task the most convenient way. Over time you get a codebase you cannot understand — she calls it **organic architecture** (written 18 Jul: [organic-architecture.md](organic-architecture.md)). The agent may still “understand” it. What it understands is a maze optimized for shortcuts.
...
- Think “Next.js for Electron, designed for agents to write.”
- Feature colocation; main vs renderer; CI dependency graph; ban `useEffect`; **ban comments**.
- Comment ban is not aesthetics. Agents write “historical” comments that are wrong for the current code — e.g. “Lauren said never do this” when she only said “this PR is bad, fix that part.” The context dies. The fragment fossilizes as a house rule.
...
| 3 | Rules, skills, style guides, AI review | Soft — forgotten |
...
Same day as the talk, Grok 4.6 shipped at 4.5 prices. She reads Cursor / xAI as hunting a **cost–intelligence sweet spot**, not the biggest model.

## Grok Bot as the non-engineer Cursor moment

Before Grok Bot, Cursor was an editor and a CLI — power-user tools. Grok Bot looks like iMessage. Named bots. Personalities. A team.

PMs at her company are already shipping: “I found a bug, I fixed it, can you look?” She reviews. It is clean. Approve. That is Dune working: **hard constraints let non-specialists contribute at a high bar.**

Constraint creates freedom. That is the close.

## What the 0xCodez caption adds (not her manuscript)

The tweet text attributes these lines to her. They match the workshop’s *shape* (named bots, manager stance, 24/7) but they are not in the TANREN/XiaoHu chapter notes as verbatim. Treat as **attributed, not transcribed**:

- 10–20 Grok Bot agents
...
| [Loops You Can Trust](loops-you-can-trust.md) | Benny “already fixed on main”; evals as unit tests; blinded scoring |
...
- Build eyes first (`control-<app>` + Feature Map). Codegen second. The map’s contract is [feature-maps.md](feature-maps.md): four H2s, attributes as API, `wait-settle` ≠ turn-done.
...
- Do not copy lab token spend. Spend to make a naive agent safe, or hire — pick one.
