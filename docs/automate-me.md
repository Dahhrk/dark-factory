...
`/poteto-mode` is the public pstack style. `/automate-me` mines **your** transcripts and drafts a personal `-mode` skill (`jay-mode`, `priya-mode`, …) that sits **on top of** pstack. You keep pstack as the base. This kitchen’s overlay is `/brooklyn-mode`.
...
4. **Draft** via built-in `create-skill`. Shape like poteto-mode — **do not paste poteto-mode’s body**. `disable-model-invocation: true` by default (invoke by name only).
