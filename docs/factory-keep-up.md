...
3. When a Cursor pack twin checkout exists (`PLUG_FACTORY_REPO` or a sibling
   `plug-factory` directory), run `scripts/drift-check.mjs` from the Devin
   plugins tree in hard-fail mode. The advisory `--content` pass alone never
   opens a PR. If no Cursor pack twin is seated, skip that script. That is
   expected until the twin exists.
